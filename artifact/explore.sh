...
echo "[artifact] Launching the Jupyter docker container"
docker run -p 8888:8888 --rm -v "${RESULTS_DIR}:/work/results:rw" -v "${INSTANCES_DIR}:/work/instances" -it --entrypoint ./jupyter_impl.sh "${IMAGE_TAG}" "results-explore.ipynb"
