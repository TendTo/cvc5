...
void TheoryArithPrivate::tryBranchCut(external::ExternalSimplex* approx,
                                      int nid,
                                      BranchCutInfo& bci)
{
  Assert(conflictQueueEmpty());
...
std::vector<ConstraintCPVec> TheoryArithPrivate::replayLogRec(
    external::ExternalSimplex* approx, int nid, ConstraintP bc, int depth)
{
  ...
  if (relaxRes == LinResult::LinFeasible)
  {
    ...
    switch (mipRes)
    {
      ...
      case MipResult::PivotsExhauasted:
        ...
        replayLemmas(approx);
        break;
      case MipResult::MipUnknown: break;
    }
  }
  delete approx;
